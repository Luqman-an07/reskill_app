...
// --- PROPS DARI CONTROLLER (SESUAI TAHAP 4) ---
...
// --- STATE ---
// Default tab: Prioritas 1 (Wajib)
const activeTab = ref('priority_1');
const search = ref('');
const devFilter = ref('Semua');

// Hanya mengambil kategori yang ada di Priority 3
const devCategories = computed(() => {
    const courses = props.sections?.priority_3 || [];
    // Ambil target_role unik, filter yang kosong/null
    const roles = courses.map(c => c.target_role).filter(r => r); 
    // Return array unik dengan 'Semua' di depan
    return ['Semua', ...new Set(roles)];
});

// Reset filter saat pindah tab agar UX bersih
watch(activeTab, () => {
...
const displayedCourses = computed(() => {
    // 1. Ambil Data Sesuai Tab Aktif
    let courses = props.sections[activeTab.value] || [];

    // [BARU] Filter Sub-Kategori (Khusus Tab Pengembangan Diri)
...
// --- STATUS GATEKEEPER INFO (ALERT) ---
const currentInfoBar = computed(() => {
    // 1. KONDISI TERKUNCI (GATEKEEPER - MERAH)
    ...
    // 2. KONDISI TERBUKA - GENERAL (INFO - BIRU)
    // Kita berikan info agar layout tidak lompat
    ...
    // 3. KONDISI TERBUKA - KOMPETENSI/DEV (SUKSES - HIJAU)
    // Jika sudah terbuka, beri semangat
    return {
        show: true,
        type: 'success', // Hijau
        icon: 'check',
                ...
                >
                    ...
                    <div>
                        <h3 class="text-sm font-bold"
                            :class="{
                                'text-red-800': currentInfoBar.type === 'locked',
                                'text-blue-800': currentInfoBar.type === 'info',
                                'text-green-800': currentInfoBar.type === 'success',
                            }"
                        >
                        ...
                        <p class="text-sm mt-0.5"
                            :class="{
                                'text-red-600': currentInfoBar.type === 'locked',
                                'text-blue-600': currentInfoBar.type === 'info',
                                'text-green-600': currentInfoBar.type === 'success',
                            }"
                        >
                            {{ currentInfoBar.msg }}
                        </p>
                            ...
                            <div class="p-5 flex flex-col flex-1">
                                ...
                                <div class="flex flex-wrap items-center gap-2 mb-3 text-xs">
                                    ...
                                    <div class="flex items-center gap-1 text-yellow-700 bg-yellow-50 px-2 py-1 rounded border border-yellow-100" title="Total Potensi Poin">
                                        ...
                                        <span v-if="course.completion_points > 0" class="text-[10px] opacity-70 ml-0.5 border-l border-yellow-200 pl-1">
                                            (+{{ course.completion_points }} Bonus)
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="mt-auto space-y-3">
                                    ...
                                    <Link 
                                        ...
                                        class="block w-full text-center py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm active:scale-95 border"
                                        :class="course.is_enrolled 
                                            ? 'bg-blue-50 text-blue-700 border-blue-100 hover:bg-blue-100' 
                                            : 'bg-white text-gray-700 border-gray-200 hover:border-gray-300 hover:bg-gray-50'"
                                    >
                                        {{ course.is_enrolled ? 'Lanjutkan Belajar' : 'Lihat Detail & Materi' }}
                                    </Link>
